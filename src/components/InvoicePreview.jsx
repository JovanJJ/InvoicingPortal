...
import { saveInvoice, splitTimeEntryByInvoice, updateTimeEntry } from '@/lib/actions';


export default function InvoicePreview({ handleInvoicePreview, project, client, timeEntries, user, getCommitMessages, bankIban }) {
    ...
    const lineItems = unbilledEntries.map(e => {
        ...
        const activeDuration = isEditing && updateSessionDuration !== "" ? Number(updateSessionDuration) * 60 : e.duration;

        return {
            id: e._id,
            description: e.description,
            date: formatDate(e.createdAt),
            hours: formatDurationForInvoice(e.duration),
            rate: project.rate,
            total: ((activeDuration / 3600) * project.rate).toFixed(2)
        };
    });
                                            ...
                                            <p
                                                onClick={async () => {
                                                    if (edditing === item.id) {
                                                        ...
                                                        setUpdateSessionDuration(Math.floor(originalEntry.duration / 60).toString());
                                                        setEdditing(item.id);
                                                    }
                                                }}
